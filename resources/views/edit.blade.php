...
@section('content')
    <div class="container h-full">
        <div class="text-lg font-bold">Edit Pengguna</div>
        <div class="mt-4 border p-4 border-gray-100 rounded-lg">
            <form>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <input type="password" name="confirm_password" id="floating_repeat_password"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder=" " required />
    ...
    <script>
        $(document).ready(function() {
            ...
            const url = "http://127.0.0.1:8004/api/api-edit/" + id
            console.log(url);
            $.ajax({
                url,
                type: 'GET',
                success: function(response) {
                    ...
                    $('input[name=fullname]').val(data.user_fullname); // Corrected property name to "user_fullname"
                    $('input[name=email]').val(data.user_email); // Corrected property name to "user_email"
                    $('input[name=password]').val(data.password); // Corrected property name to "password"
            ...
            $('form').submit(function(event) {
                ...
                $.ajax({
                        type: 'POST',
                        url: 'http://127.0.0.1:8004/api/api-edit-post/' + id,
                        data: formData,
                        dataType: 'json',
                        encode: true
                    })
